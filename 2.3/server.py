...
def login():
    if request.method == "POST":
        ...
        encrypted_keys_b64 = request.form['session_keys']
        ciphertext_b64 = request.form['ciphertext']
        hmac_b64 = request.form['hmac']

        #! Decode, decrypt and parse session keys
        encrypted_keys = b64decode(encrypted_keys_b64.encode('ascii'))
        ...
        session_keys = private_key.decrypt(
            encrypted_keys,
            padding.OAEP(
        ...
        hmac_server = hmac.new(
            mac_key, (encrypted_keys + ciphertext), sha256).digest()
...
def crt():
    with open('10770408_auto.crt', 'r') as crt_file:
        crt = crt_file.read()
    return crt
